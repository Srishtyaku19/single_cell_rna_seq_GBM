...
leblanc.expmat <- readRDS("LeBlancExpMat.RDS")
```
...
#Genes expressed in less than 200 cells are removed. Cells with at least 7000 detected genes are kept.
leblanc <- CreateSeuratObject(counts = leblanc.expmat, min.cells = 200, min.features = 2000)
...
```{r}
#leblanc.matrix <- GetAssayData(object = leblanc , slot = "scale.data")
#View(leblanc.matrix)
#write.table(leblanc.matrix, "leblancmatrix2002000(data).txt")
#exp.matFINALleblanc <- read.table(file = "leblancmatrix2002000(scaled).txt", header = TRUE, as.is = TRUE, row.names = 1)
```
...
dimUsed <- 20
leblanc.tsne <- RunTSNE(leblanc, dims=1:dimUsed, seed.use=2021)

DimPlot(leblanc.tsne,reduction = "tsne", label = TRUE)
```

```{r}
dimUsed <- 20
leblanc <- RunUMAP(leblanc, dims=1:dimUsed, seed.use=2021)
DimPlot(leblanc,reduction = "umap", label = TRUE)
```

Differential Expression
```{r}
G1.clusters <- FindMarkers(leblanc, ident.1 = "G1",ident.2 = c("G2M","S") , min.pct = 0.25,logfc.threshold = 0.5)
...
```{r}
G2M.clusters <- FindMarkers(leblanc, ident.1 = "G2M" ,ident.2 = c("G1","S"), min.pct = 0.25,logfc.threshold = 0.5)
...
```{r}
S.clusters <- FindMarkers(leblanc, ident.1 = "S",ident.2 = c("G1","G2M") , min.pct = 0.25,logfc.threshold = 0.5)
#View(S.clusters)
...
leblanc.markers <- FindAllMarkers(leblanc, only.pos = FALSE, min.pct = 0.25, logfc.threshold = 0.5)
leblanc.markers <- subset(leblanc.markers, leblanc.markers$p_val_adj < 0.05)

#View(leblanc.markers)

write_xlsx(leblanc.markers,"leblancDEanalysis.xlsx")

#saveRDS(leblanc.markers, "leblancDElist.RDS")


markers.top <- leblanc.markers[order(-leblanc.markers$avg_log2FC),]
#markers.top

markers.bottom <- leblanc.markers[order(leblanc.markers$avg_log2FC),]
#markers.bottom
```
...
# Overlap between DE genes and tirosh marker genes # 64/96
length(intersect(rownames(leblanc.markers),s.genes)) # 17/43
length(intersect(rownames(leblanc.markers),g2m.genes)) # 47/53
```

```{r}
# Visualize the distribution of cell cycle markers across (G1 phase)
RidgePlot(leblanc, features = top5$gene[1:5], ncol = 3)
RidgePlot(leblanc, features = bottom5$gene[1:5], ncol = 3)
```


```{r}
# Visualize the distribution of cell cycle markers across (S phase)
RidgePlot(leblanc, features = top5$gene[6:10], ncol = 3)
RidgePlot(leblanc, features = bottom5$gene[6:10], ncol = 3)
```


```{r}
# Visualize the distribution of cell cycle markers across (G2/M phase)
RidgePlot(leblanc, features = top5$gene[11:15], ncol = 3)
RidgePlot(leblanc, features = bottom5$gene[11:15], ncol = 3)
```
...
RidgePlot(leblanc, features = "TACC3", ncol = 1)
```


```{r}
#G1 PHASE
FeaturePlot(leblanc, features = top5$gene[1:5], ncol=3)
FeaturePlot(leblanc, features = bottom5$gene[1:5], ncol=3)
```

```{r}
#S PHASE
FeaturePlot(leblanc, features = top5$gene[6:10], ncol=3)
FeaturePlot(leblanc, features = bottom5$gene[6:10], ncol=3)
```

```{r}
#G2M PHASE
FeaturePlot(leblanc, features = top5$gene[11:15], ncol=3)
FeaturePlot(leblanc, features = bottom5$gene[11:15], ncol=3)
```

```{r}
# FeaturePlot for TACC3
FeaturePlot(leblanc, features = "TACC3", ncol=1)
```
...
```{r}
#G1 PHASE
VlnPlot(leblanc, features = top5$gene[1:5], ncol =3)
VlnPlot(leblanc, features = bottom5$gene[1:5], ncol =3)
```

```{r}
#S PHASE
VlnPlot(leblanc, features = top5$gene[6:10], ncol =3)
VlnPlot(leblanc, features = bottom5$gene[6:10], ncol =3)
```

```{r}
#G2M PHASE
VlnPlot(leblanc, features = top5$gene[11:15], ncol =3)
VlnPlot(leblanc, features = bottom5$gene[11:15], ncol =3)
```

```{r}
# Violin Plot for TACC3
VlnPlot(leblanc, features = "TACC3", ncol =1)
```


```{r}
DotPlot(leblanc, features = unique(top5$gene) ,dot.scale = 8) + RotatedAxis()
DotPlot(leblanc, features = unique(bottom5$gene) ,dot.scale = 8) + RotatedAxis()
```
...
write_xlsx(g1.fgseaRes.sig,"g1.fgseaRes.sig.xlsx")
```

```{r}
#Create pathway specific enrichment plot
# pdf("G1fGsea.EnrichmentPlt(GO)leblanc.pdf",width=3,height=3)
# for(i in 1:2)
# {
...
write_xlsx(g2m.fgseaRes.sig,"g2m.fgseaRes.sig.xlsx")
```


```{r}
#Create pathway specific enrichment plot
# pdf("G2MfGsea.EnrichmentPlt(GO)leblanc.pdf",width=3,height=3)
# for(i in 1:2)
# {
...
```{r}
#Create pathway specific enrichment plot
# pdf("SfGsea.EnrichmentPlt(GO)leblanc.pdf",width=3,height=3)
# for(i in 1:2)
# {
...
```{r}
#save.image(file = "Leblanc 200 2000.RData")
```
